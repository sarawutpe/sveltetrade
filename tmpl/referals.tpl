          {include file="header.tpl" meta_title="Referrals"}
          <span class="fontSize-xl fontWeight-regular mb-3">Referrals</span>

          {if $upline.email != ""}
            <span class="fontSize-mPlus mb-3">Your upline is <a href=mailto:{$upline.email}>{$upline.name}</a></span>
          {/if}

          {if $settings.use_referal_program == 1}
            <fluent-card class="card flex justify-content-between">
              <span class="fontSize-mPlus fontWeight-regular">Referral link</span>
              <a class="fontSize-m" href="{$settings.site_url}/?ref={$userinfo.username}">
                {$settings.site_url}/?ref={$userinfo.username}
              </a>
            {/if}
          </fluent-card>
          <fluent-card class="card flex justify-content-between">
            <span class="fontSize-mPlus fontWeight-regular">Referrals</span>
            <span class="fontSize-m">{$total_ref}</span>
          </fluent-card>
          <fluent-card class="card flex justify-content-between">
            <span class="fontSize-mPlus fontWeight-regular">Active referrals</span>
            <span class="fontSize-m">{$active_ref}</span>
          </fluent-card>
          <fluent-card class="card flex justify-content-between">
            <span class="fontSize-mPlus fontWeight-regular">Total referral commission</span>
            <span class="fontSize-m">{$currency_sign}{$commissions}</span>
          </fluent-card>
            ...
            <table cellspacing=0 cellpadding=1 border=0>
              <form method=post name=opts>
                ...
                <td>
                  <select name=month_from class=inpts>
                    {section name=month_from loop=$month}
                      <option value={$smarty.section.month_from.index+1}
                        {if $smarty.section.month_from.index+1 == $frm.month_from}selected{/if}>{$month[month_from]}
                      {/section}
                  </select> &nbsp;
                  <select name=day_from class=inpts>
                    {section name=day_from loop=$day}
                      <option value={$smarty.section.day_from.index+1}
                        {if $smarty.section.day_from.index+1 == $frm.day_from}selected{/if}>{$day[day_from]}
                      {/section}
                  </select> &nbsp;
                  <select name=year_from class=inpts>
                    {section name=year_from loop=$year}
                      <option value={$year[year_from]} {if $year[year_from] == $frm.year_from}selected{/if}>
                        {$year[year_from]}
                      {/section}
                  </select>
                </td>
                ...
                <tr>
                  ...
                  <td> <select name=month_to class=inpts>
                      {section name=month_to loop=$month}
                        <option value={$smarty.section.month_to.index+1}
                          {if $smarty.section.month_to.index+1 == $frm.month_to}selected{/if}>{$month[month_to]}
                        {/section}
                    </select> &nbsp;
                    <select name=day_to class=inpts>
                      {section name=day_to loop=$day}
                        <option value={$smarty.section.day_to.index+1}
                          {if $smarty.section.day_to.index+1 == $frm.day_to}selected{/if}>{$day[day_to]}
                        {/section}
                    </select> &nbsp;
