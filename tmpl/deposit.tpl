            ...
            <div class="grid-container grid-column-2">
              ...
              <div class="form-control">
                ...
                <div class="field">
                  ...
                  <fluent-radio name="h_id" value="{$plan.id}" {if (($smarty.foreach.fplans.first == 1) && (!$frm.h_id)) || ($frm.h_id == $plan.id)}checked{/if}>{$plan.name|escape:html}</fluent-radio>
                  {/foreach}
                  </fluent-radio-group>
                </div>
                {* Plan Details *}
                {foreach from=$plan.plans item=o}
                  <div>
                    <fluent-card class="card">
                      <span id="plan-name" class="fontSize-mPlus fontWeight-regular mb-2">{$o.name|escape:html}</span>
                      <div class="flex align-items-center">
                        <i class="ms-Icon ms-Icon--CheckMark fontSize-m mb-2 mr-1"></i>
                        <span id="plan-min" class="fontSize-sPlus mb-2">Min {$o.min_deposit|fiat}</span>
                      </div>
                      <div class="flex align-items-center">
                        <i class="ms-Icon ms-Icon--CheckMark fontSize-m mb-2 mr-1"></i>
                        <span id="plan-max" class="fontSize-sPlus mb-2">Max {if $o.max_deposit == 0}&infin;{else}{$o.max_deposit|fiat}{/if}</span>
                      </div>
                    </fluent-card>
                  </div>
                {/foreach}
                ...
                <div class="field mb-3">
                  ...
                  <fluent-option value="process_{$p.id}" {if $frm.type == "process_`$p.id`"}selected{/if}>Topup {$p.name}</fluent-option>
                  <fluent-option value="account_{$p.id}" {if $frm.type == "account_`$p.id`"}selected{/if} {if $p.available <= 0}disabled{/if}>Balance {$p.name} {$p.available|fiat:$p.id}</fluent-option>
                  {/foreach}
                  </fluent-select>
                </div>
                {* Amount *}
                ...
                <div class="field">
                  <fluent-number-field id="input-amount" class="w-100 fontSize-mPlus fontWeight-regular mb-2" name="amount" value="{$frm.amount|default:$min_deposit|amount_format}" required></fluent-number-field>
                </div>
                {* Compounding *}
                <div class="field">
