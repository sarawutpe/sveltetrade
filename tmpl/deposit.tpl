          ...
          {else}
            {if $errors}
              ...
              {/if}
              {if $errors.deposits_num_exeeded}
                <div class="alert error">
                  Sorry, this plan limit {$errors.deposits_num_exeeded.limit|escape:html} active deposit.
                </div>
              {/if}
              ...
              <div class="grid-container grid-column-2">
                ...
                <div class="form-control">
                        ...
                        {if $plans|@count > 1}
                          <div class="card-plan-content">
                            ...
                            {if $plan.plans|@count > 0}
                              {$plan_item_calc = [{$plan.q_days},{$plan.plans[0].percent},{$plan.plans[0].min_deposit},{$plan.plans[0].max_deposit},
                              {$plan.period}]}
                            {/if}

                            <input aria-valuetext={$plan_item_calc|json_encode} id="{$plan.id}" type="radio" class="card-plan"
                              name="h_id" value="{$plan.id}"
                              {if (($smarty.foreach.fplans.first == 1) && (!$frm.h_id)) || ($frm.h_id == $plan.id)} checked
                              {/if}>
                            {foreach from=$plan.plans item=o}
                              <label class="label" for="{$plan.id}">
                                <fluent-card class="card card-plan-item">
                                  <span id="plan-name"
                                    class="fontSize-mPlus fontWeight-semibold mb-2">{$o.name|escape:html}</span>
                  ...
                  <div class="field mb-3">
                    <fluent-select {if $dps|@count < 0}disabled{/if} class="w-100 z-10" name="type" position="below">
                  ...
                  <div class="flex justify-content-between">
                    ...
                    {if $dps}
                      <fluent-badge id="select-min-amount" class="cursor-pointer" appearance="accent">
                        SELECT MIN SPEND
                      </fluent-badge>
                    {/if}
                  </div>
                  <div class="field">
                    <fluent-number-field {if $dps|@count < 0}disabled{/if} id="input-amount"
                      class="w-100 fontSize-mPlus fontWeight-regular mb-2" name="amount"
                      value="{$frm.amount|default:$min_deposit|amount_format}" required>
                    </fluent-number-field>
                  </div>
                  {* Compounding *}
                  <div class="field">
