                ...
                <div class="form-control">
                  ...
                  <div class="field">
                    <fluent-radio-group orientation="vertical">
                      {if $plans|@count > 1}
                        <input type=radio name=h_id value="{$plan.id}"
                  ...
                  {foreach from=$plans item=plan name=fplans}
                    <fluent-radio name="h_id" value="{$plan.id}"
                      {if (($smarty.foreach.fplans.first == 1) && (!$frm.h_id)) || ($frm.h_id == $plan.id)}checked{/if}>
                      {$plan.name|escape:html}</fluent-radio>
                  {/foreach}
                  ...
                  {foreach from=$plan.plans item=o}
                    <div>
                      <fluent-card class="card">
                        ...
                        <div class="flex align-items-center">
                          <i class="ms-Icon ms-Icon--CheckMark fontSize-m mb-2 mr-1"></i>
                          <span id="plan-max" class="fontSize-sPlus mb-2">Max
                            {if $o.max_deposit == 0}&infin;{else}{$o.max_deposit|fiat}{/if}</span>
                        </div>
                      </fluent-card>
                    </div>
                  {/foreach}
