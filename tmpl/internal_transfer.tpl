...
{if $say == 'processed'}
<div class="alert success">Internal transfer has been successfully completed.</div>
{/if}

{if $fatals}

{if $fatals.forbidden}<div class="alert error">Internal transfers are forbidden.</div>{/if}
{if $fatals.no_deposits}<div class="alert error">You can not send funds before you make any deposit.</div>{/if}
{if $fatals.times_limit}<div class="alert error">You can send internal transfer {$settings.limit_transfer_period_times} per {$settings.limit_transfer_period_date} only.</div>{/if}

{else}
{if $errors}
{if $errors.turing_image}<div class="alert error">Invalid turing image</div>{/if}
{if $errors.user_not_found}<div class="alert error">The recipient's username entered has not been found or has been suspended.</div>{/if}
{if $errors.less_min}<div class="alert error">You can transfer the amount not less {$errors.less_min} only.</div>{/if}
{if $errors.too_big_amount}<div class="alert error">You have no such amount on your balance.</div>{/if}
{if $errors.too_big_amount_plus_fee}<div class="alert error">You have no enough funds to complte the transaction. Total amount you should have to send {$frm.amount|fiat:$frm.ec} + fee is <b>{$errors.too_big_amount_plus_fee}</b>.</div>{/if}
{if $errors.greater_max}<div class="alert error">Maximum amount you can send is {$errors.greater_max}.</div>{/if}
{if $errors.invalid_transaction_code}<div class="alert error">Invalid Transaction Code.</div>{/if}
{if $errors.invalid_tfa_code}<div class="alert error">You have entered invalid 2FA code.</div>{/if}
{/if}
