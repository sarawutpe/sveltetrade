                ...
                <div class="mb-1">
                  ...
                  <fluent-select class="select" position="below" name="type" onchange="document.opts.submit();">
                    ...
                    {section name=opt loop=$options}
                      <fluent-option value="{$options[opt].type}" {if $options[opt].selected}selected{/if}>
                        {$options[opt].type_name}</fluent-option>
                    {/section}
                  </fluent-select>
                </div>
                {if $ecs}
                  ...
                  <div class="mb-1">
                    <fluent-select class="select" name="ec" position="below" onchange="document.opts.submit();">
                      ...
                      {section name=ec loop=$ecs}
                        <fluent-option value={$ecs[ec].id} {if $frm.ec eq $ecs[ec].id}selected{/if}>{$ecs[ec].name}
                        </fluent-option>
                      {/section}
                    </fluent-select>
                  </div>
                ...
                <div class="mb-1">
                  ...
                  <fluent-select class="select small" position="below" name="month_from"
                    ...
                    {section name=month_from loop=$month}
                      <fluent-option value={$smarty.section.month_from.index+1}
                  ...
                  <fluent-select class="select small" position="below" name="day_from"
                    ...
                    {section name=day_from loop=$day}
                      <fluent-option value={$smarty.section.day_from.index+1}
                  ...
                  <fluent-select class="select small" position="below" name="year_from"
                    ...
                    {section name=year_from loop=$year}
                      <fluent-option value={$year[year_from]} {if $year[year_from] == $frm.year_from}selected{/if}>
                        {$year[year_from]}</fluent-option>
                    {/section}
                  </fluent-select>
                ...
                <div class="mb-1">
                  ...
                  <fluent-select class="select small" position="below" name="month_to"
                    ...
                    {section name=month_to loop=$month}
                      <fluent-option class="option" value={$smarty.section.month_to.index+1}
                  ...
                  <fluent-select class="select small" position="below" name="day_to" onchange="document.opts.submit();">
                    {section name=day_to loop=$day}
                      <fluent-option class="option" value={$smarty.section.day_to.index+1}
                        {if $smarty.section.day_to.index+1 == $frm.day_to}selected{/if}>{$day[day_to]}</fluent-option>
                    {/section}
                  </fluent-select>
                  <fluent-select class="select small" position="below" name="year_to"
                    onchange="document.opts.submit();">
                    {section name=year_to loop=$year}
                      <fluent-option class="option" value={$year[year_to]}
                        {if $year[year_to] == $frm.year_to}selected{/if}>{$year[year_to]}</fluent-option>
                    {/section}
                  </fluent-select>
                </div>
              </div>
                  ...
                  <tr>
                    ...
                    <td>
                      <div class="flex align-items-center">
                        ...
                        <img class="img-currency" src="images/{$t.ec}.gif">{if $t.type == 'withdraw_pending'}<a
                            href="{"?a=cancelwithdraw&id=`$t.id`&return=`$return`&"|encurl}"
                          onclick="return confirm('Are you sure you want to delete this request?')">[cancel]</a>{/if}
                      </div>
                    </td>
